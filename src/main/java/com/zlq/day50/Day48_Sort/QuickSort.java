...
//        int[] arr = {3,6};
	...
	private static int partition(int[] arr, int leftBound, int rightBound) {
		...
		int l = leftBound, r = rightBound - 1, pivot = arr[rightBound];
		while (l < r) {
			while (arr[l] <= pivot && l <= r) {
				l++;  // 目的：让l指针找到比pivot大的值，找不到就右移
			}
			while (arr[r] > pivot && l < r) {
				r--;  // 目的：让r指针找到比pivot小的值，找不到就左移
			}
			if (l < r) {
				swap(arr, l, r);    // 交换l和r，最终，比pivot小的值在左边，比pivot大的值在右边
			}
		}
